...
class InstallingModelScreen(ModalScreen[bool]):
    ...
    def start_installation(self) -> None:
        ...
        try:
            ...
            def progress_callback(status: str, percentage: int):
                self.app.call_from_thread(setattr, self, "status_message", status)
                self.app.call_from_thread(setattr, self, "progress_percentage", percentage)
...
class InstallModelScreen(ModalScreen[str]):
        ...
        with Container(id="install-dialog"):
            ...
            with ListView(id="install-list"):
                for model_info in get_recommended_models():
                    ...
                    item_with_attrs = cast(Any, item)
                    item_with_attrs.model_name = name
                    item_with_attrs.is_installed = is_installed
                    yield item
    ...
    def on_list_view_selected(self, event: ListView.Selected) -> None:
        ...
        if hasattr(event.item, "model_name"):
            self.selected_model = cast(str, cast(Any, event.item).model_name)
        ...
        if event.button.id == "install-button":
            if self.selected_model:
                ...
                if list_view.highlighted_child and hasattr(list_view.highlighted_child, "is_installed"):
                    if bool(cast(Any, list_view.highlighted_child).is_installed):
...
class SettingsScreen(Screen):
    ...
    def render_ai_section(self) -> list:
        ...
        for provider_id, provider_name, provider_desc in providers:
            ...
            cast(Any, btn).provider_id = provider_id
            widgets.append(btn)
            if is_current:
    ...
    def render_openai_settings(self) -> list:
        ...
        for model_id, model_name, model_desc in models:
            ...
            cast(Any, btn).model_id = model_id
            widgets.append(btn)
            if is_current:
        ...
        try:
            ...
            if not installed:
                ...
                for model in installed:
                    ...
                    cast(Any, btn).model_name = model.name
                    widgets.append(btn)
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        # Provider selection
            ...
            if hasattr(event.button, "provider_id"):
                provider_id = cast(str, cast(Any, event.button).provider_id)
    ...
    async def handle_installation_complete(self, success: bool | None) -> None:
    ...
    def action_save(self) -> None:
        ...
        def input_value(selector: str) -> Optional[str]:
            inputs = self.query(selector)
            if not inputs:
                return None
            return cast(Input, inputs[0]).value.strip()

        # Update config from inputs (only if they're currently mounted)
        try:
            value = input_value("#notes-dir-input")
            if value is not None:
                self.config["notes_dir"] = value
        except Exception:
            pass

        try:
            value = input_value("#rec-dir-input")
            if value is not None:
                self.config["recordings_dir"] = value
        except Exception:
            pass

        try:
            value = input_value("#trans-dir-input")
            if value is not None:
                self.config["transcripts_dir"] = value
        except Exception:
            pass

        try:
            value = input_value("#editor-input")
            if value is not None:
        ...
        try:
            value = input_value("#terminal-file-browser-input")
            if value is not None:
                self.config["terminal_file_browser"] = value
        except Exception:
            pass

        # Update API keys from inputs
        try:
            value = input_value("#openai-key-input")
            if value is not None:
        ...
        try:
            value = input_value("#anthropic-key-input")
            if value is not None:
        ...
        try:
            value = input_value("#openrouter-key-input")
            if value is not None:
