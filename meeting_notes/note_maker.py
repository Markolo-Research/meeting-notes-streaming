...
try:
    from .ai_summarizer import AnthropicSummarizer, MeetingSummary, OpenAISummarizer, OpenRouterSummarizer
...
class NoteMaker:
    ...
    def _extract_simple_summary(self, text: str) -> dict:
        ...
        clean_words = [word.lower().strip('.,!?;:()[]{}""\'') for word in words if len(word) > 3]

        filtered_words = [word for word in clean_words if word not in stop_words and word.isalpha()]
    ...
    def _generate_transcript_file(
        self, title: str, date: datetime, duration: float, formatted_transcript: str, recording_file: str
    ) -> str:
...
**[00:10]** I think we should focus on the API endpoints first this sprint."""
    ...
    note_path, transcript_path, error = maker.create_note(
        transcript_text=test_transcript, formatted_transcript=formatted, duration=125.0, title="Sprint Planning"
    )
