...
RECOMMENDED_MODELS = [
    {"name": "llama3.2:1b", "description": "Fast, lightweight", "size": "1.3 GB", "recommended": False},
    {"name": "llama3.2:3b", "description": "Recommended balance", "size": "2.0 GB", "recommended": True},
    {"name": "llama3.1:8b", "description": "Better quality, slower", "size": "4.7 GB", "recommended": False},
    {"name": "qwen2.5:7b", "description": "Alternative option", "size": "4.7 GB", "recommended": False},
]
...
def install_model(model_name: str, progress_callback: Optional[Callable[[str, int], None]] = None) -> bool:
    ...
    try:
        ...
        process = subprocess.Popen(
            ["ollama", "pull", model_name], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1
        )
