...
class AudioRecorder:
    ...
    def __init__(self, output_dir: str | Path = "recordings", mode: str = "combined", dev_mode: bool = False):
        """
    ...
    def _start_single_recording(self, target: Optional[str], channels: int) -> str:
        ...
        if shutil.which("pw-record"):
            cmd = ["pw-record", f"--channels={channels}", "--format=s16", "--rate=48000"]
            if target:
    ...
    def _start_combined_recording(self) -> str:
        ...
        self.system_process = subprocess.Popen(system_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        if len(self.temp_files) == 2 and all(f.exists() for f in self.temp_files):
            ...
            if shutil.which("ffmpeg"):
                ...
                mix_cmd = [
                    "ffmpeg",
                    "-i",
                    str(self.temp_files[0]),  # mic
                    "-i",
                    str(self.temp_files[1]),  # system
                    "-filter_complex",
